  ...
  try {
    const resp = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a research assistant. Always respond with valid JSON only — no markdown fences, no explanation.'
          },
          {
            role: 'user',
            content: prompt
    ...
    if (!resp.ok) {
      ...
      return {
        ...
        body: JSON.stringify({ error: err.error?.message || `HTTP ${resp.status}` })
      };
    }
    ...
    if (start === -1 || end === -1) {
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'Could not parse JSON from response', raw: raw.slice(0, 300) })
      };
    }
